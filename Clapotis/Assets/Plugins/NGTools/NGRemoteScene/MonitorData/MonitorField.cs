...
namespace NGTools.NGRemoteScene
{
	internal sealed class MonitorField : MonitorData
	{
		...
		{
			//UnityEngine.Debug.Log("F"+this.fieldInfo.Name + "=" + this.value + " <> "+ this.fieldInfo.GetValue(this.getInstance()));
		...
		{
			return new Packet[] { new NotifyFieldValueUpdatedPacket(this.path, this.typeHandler.Serialize(this.fieldInfo.FieldType, this.value)) };
		}
	}
}