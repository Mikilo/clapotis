...
namespace NGToolsEditor.NGConsole
{
	...
	public sealed class LogConditionParser : ILogContentGetter
	{
		...
		private static readonly Frame[]		nullFrame = {};
		private static readonly List<Frame>	arrayFrames = new List<Frame>(8);
		private static IStackFrameFilter[]	stackFrameFilters;
		...
		public void	ParseLog()
		{
			...
			//	Vector2	size = HQ.Settings.log.style.CalcSize(Utility.content);
		...
		public void	ParseStackTrace()
		{
			...
			else
			{
				...
				for (int i = 0; i < frames.Length; i++)
				{
					...
					if (HQ.Settings.Get<GeneralSettings>().filterUselessStackFrame == true)
					{
						if (LogConditionParser.stackFrameFilters == null)
						{
							List<IStackFrameFilter>	filteredStackFrameFilters = new List<IStackFrameFilter>(2);

							foreach (Type c in Utility.EachNGTAssignableFrom(typeof(IStackFrameFilter)))
								filteredStackFrameFilters.Add((IStackFrameFilter)Activator.CreateInstance(c));

							LogConditionParser.stackFrameFilters = filteredStackFrameFilters.ToArray();
						}

						for (int j = 0; j < LogConditionParser.stackFrameFilters.Length; j++)
						{
							if (LogConditionParser.stackFrameFilters[j].Filter(frames[i]) == true)
							{
								//Debug.Log("Skipped");
		...
		public bool	ParseCompileLog()
		{
				...
				(this.log.mode & Mode.ScriptCompileWarning) != 0)
			{
				this.frames = new Frame[1] {
					new Frame() {
						frameString = Utility.Color(this.log.file, HQ.Settings.Get<StackTraceSettings>().filepathColor) + " " + Utility.Color(this.log.line.ToString(), HQ.Settings.Get<StackTraceSettings>().lineColor),
								  ...
								  Mode.ScriptingAssertion)) != 0)
			{
				...
				while (start != -1)
				{
					...
					if (start != -1)
					{
						...
						{
							int	j = this.log.condition.LastIndexOf("UnityEngine.Debug:", end, end - start);
							if (j != -1)
							{
								// Extract the stack trace and remove the last new line.
								this.stackTrace = this.log.condition.Substring(j, this.log.condition.Length - j - 1);
						...
						else
						{
							...
							if (k != -1)
							{
								...
								if ((this.log.condition.IndexOf(":", start, k - start) > 0 || this.log.condition.IndexOf(".", start, k - start) > 0) && // A colon should be present before the method name.
									this.log.condition.IndexOf(":", k, end - k) == -1 &&
		...
		private ConstructorInfo	GetConstructorFromParameters(Type classType, string[] parameters)
		{
			...
			foreach (ConstructorInfo c in classType.GetConstructors(~BindingFlags.Default))
			{
			...
			else
			{
				...
				foreach (ConstructorInfo c in matchingParamContructors)
				{
