...
namespace NGToolsEditor.NGSyncFolders
{
	...
	{
		public static readonly Color	HighlightBackground = Color.gray * .7F;
		...
		public void	ScanDiff(File model, string path, List<FilterText> filters)
		{
			if (EditorUtility.DisplayCancelableProgressBar(NGSyncFoldersWindow.Title, "Scanning slave " + NGSyncFoldersWindow.slave + " " + path, NGSyncFoldersWindow.progress))
			...
			if (dirs != null)
			{
				for (int i = 0; i < dirs.Length; i++)
				{
					...
					InternalNGDebug.InternalLogFormat("D {0}, with {1}", dirs[i], match);

					File	f = new File(this, dirs[i], name, false, true, InitialState.Origin, SlaveState.Exist);
					f.ScanDiff(match, dirs[i], filters);
					this.children.Add(f);
				}
			}
			...
			if (files != null)
			{
				...
				{
					if (filters.IsKeywordFilteredInBy(files[i]) == false)
						continue;
					...
					InternalNGDebug.InternalLogFormat("F {0}, with {1}", files[i], match);

					File	f = new File(this, files[i], name, false, false, match != null ? InitialState.Origin : InitialState.New, SlaveState.Exist);
			...
			for (int i = 0; i < currentModels.Count; i++)
			{
				...
				{
					File	f = new File(this, Path.Combine(path, currentModels[i].name), currentModels[i].name, false, true, InitialState.Origin, SlaveState.Exist);
					f.ScanDiff(currentModels[i], currentModels[i].path.Replace(model.path, this.path), filters);
					this.children.Add(f);
				}
				else
					this.children.Add(new File(this, Path.Combine(path, currentModels[i].name), currentModels[i].name, false, false, InitialState.Missing, SlaveState.NonExist));
			}
		...
		public void	ScanFolder(string path, List<FilterText> filters)
		{
			if (EditorUtility.DisplayCancelableProgressBar(NGSyncFoldersWindow.Title, "Scanning master " + path, NGSyncFoldersWindow.progress) == true)
				throw new AbortScanException();
			...
			for (int i = 0; i < dirs.Length; i++)
			{
				if (filters.IsKeywordFilteredInBy(dirs[i]) == false)
					continue;

				InternalNGDebug.InternalLogFormat("D {0}", dirs[i]);

				File	f = new File(this, dirs[i], Path.GetFileName(dirs[i]), true, true, InitialState.Origin, MasterState.Same);
				f.ScanFolder(dirs[i], filters);
				this.children.Add(f);
			}
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				InternalNGDebug.InternalLogFormat("F {0}", files[i]);

				File	f = new File(this, files[i], Path.GetFileName(files[i]), true, false, InitialState.Origin, MasterState.Same);
				this.children.Add(f);
			}
		...
		private void	CheckDisplay(File model)
		{
			...
			for (int j = 0; j < this.children.Count && this.canDisplay == false; j++)
			{
				...
				if (this.isMaster == true)
				{
					if (this.children[j].masterState != MasterState.Same)
		...
		private Action	ChooseAction(File m)
		{
			if (m != null)
			{
				if (m.masterState == MasterState.Same)
				{
					if (this.slaveState == SlaveState.NonExist)
				...
				else if (m.masterState == MasterState.Created)
				{
					if (this.slaveState == SlaveState.NonExist)
						return Action.Create;
					else if (this.initialHash == m.initialHash)
				...
				else if (m.masterState == MasterState.Altered)
				{
					if (this.slaveState == SlaveState.NonExist)
						return Action.Create;
				...
				else if (m.masterState == MasterState.Deleted)
				{
					if (this.slaveState == SlaveState.NonExist)
						return Action.Nothing;
					else
				...
				this.actionException = new NotImplementedException("MasterState " + Enum.GetName(typeof(MasterState), m.masterState) + " is unknown.");
