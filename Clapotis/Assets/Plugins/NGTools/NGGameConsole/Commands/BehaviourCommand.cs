...
namespace NGTools.NGGameConsole
{
	public class BehaviourCommand : CommandNode
	{
		public	BehaviourCommand(string name, string description, object instance) : base(instance, name, description)
		{
			...
			{
				if (fields[i].IsDefined(typeof(CommandAttribute), false) == true)
			...
			for (int i = 0; i < properties.Length; i++)
			{
				...
				if (attr.Length > 0)
				{
					...
					try
					{
						this.AddChild(new MemberCommand(attr[0], new PropertyModifier(properties[i]), this.instance));
					}
					catch (NotSupportedMemberTypeException ex)
					{
						InternalNGDebug.LogError(Errors.CLI_UnsupportedPropertyType, "Property \"" + ex.member.Name + "\" of type \"" + ex.member.Type.Name + "\" from \"" + instance.GetType().Name + "\" alias \"" + name + "\" is unsupported.");
					}
				}
			}
