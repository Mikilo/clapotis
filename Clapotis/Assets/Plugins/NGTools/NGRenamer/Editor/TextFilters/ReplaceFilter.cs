	...
	internal sealed class ReplaceFilter : TextFilter
	{
		...
		public override void	Highlight(string input, List<int> highlightedPositions)
		{
			...
			if (this.regex == false)
			{
				if (this.caseSensitive == true)
					this.HighlightPattern(input, this.pattern, StringComparison.Ordinal, highlightedPositions);
				else
					this.HighlightPattern(input, this.pattern, StringComparison.OrdinalIgnoreCase, highlightedPositions);
			}
			else
			{
		...
		public override string	Filter(string input)
		{
			...
			if (this.regex == false)
			{
				...
				return this.Replace(input, this.pattern, this.replace, StringComparison.OrdinalIgnoreCase);
			}
			else
			{
				RegexOptions	options = RegexOptions.Multiline;
