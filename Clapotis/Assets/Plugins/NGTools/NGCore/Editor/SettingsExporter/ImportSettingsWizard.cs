...
namespace NGToolsEditor
{
	...
	public class ImportSettingsWizard : ScriptableWizard
	{
		...
		private class ImportNode
		{
			...
			public ArrayImportOption	arrayImportOption;
			...
			public bool	HasParentAddImportOption()
			{
				...
				while (n != null)
				{
					if (n.parent != null && // Remove options from root and fields,
						...
						n.arrayImportOption == ArrayImportOption.Add)
					{
						return true;
					}
		...
		private void	Import(ImportNode node)
		{
			...
			if (node.parent != null)
			{
				...
				if (node.fieldInfo != null)
				{
							 ...
							 node.instance == null)
					{
						...
						else
						{
							...
							{
								InternalNGDebug.Log("Array.CreateInstance(" + node.instanceType + ") " + Enum.GetName(typeof(ExportableAttribute.ArrayOptions), node.arrayExportOptions) + "	" + node.instance + "	" + node.node.value);
								node.instance = Array.CreateInstance(node.fieldInfo.Type.GetElementType(), 0);
							}
							else
							{
								InternalNGDebug.Log("Activator.CreateInstance(" + node.instanceType + ") " + Enum.GetName(typeof(ExportableAttribute.ArrayOptions), node.arrayExportOptions) + "	" + node.instance + "	" + node.node.value);
				...
				else if (node.arrayElementType != null)
				{
					...
					if ((node.HasParentAddImportOption() == true || node.arrayImportOption == ArrayImportOption.Add) &&
					...
					else if (node.arrayImportOption == ArrayImportOption.Overwrite ||
		...
		private static Type	GetElementTypeOfEnumerable(object o)
		{
			...
			{
				if (interfaces[i].IsGenericType == true && interfaces[i].GetGenericTypeDefinition() == typeof(IEnumerable<>))
