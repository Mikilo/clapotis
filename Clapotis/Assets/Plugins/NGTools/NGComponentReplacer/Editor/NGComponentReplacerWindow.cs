...
namespace NGToolsEditor.NGComponentReplacer
{
	...
	public class NGComponentReplacerWindow : EditorWindow, IHasCustomMenu
	{
					...
					for (int j = 0; j < data.links.Count; j++)
					{
						...
						else
						{
							for (int k = 0; k < data.converters.Length; k++)
							{
								...
								{
									InternalNGDebug.Log("Converting " + from.Type + " to " + to.Type);
		...
		public const string	Title = "NG Component Replacer";
		public static Color	TitleColor = new Color(165F / 255F, 42F / 255F, 42F / 255F, 1F); // Brown
		...
		private ErrorPopup	errorPopup = new ErrorPopup(NGComponentReplacerWindow.Title, "An error occurred, try to reopen " + NGComponentReplacerWindow.Title + ".");
		...
		//[MenuItem(Constants.MenuItemPath + NGComponentReplacerWindow.Title + "	[ALPHA]", priority = Constants.MenuItemPriority + 350)]
		public static void	Open()
		{
			Utility.OpenWindow<NGComponentReplacerWindow>(NGComponentReplacerWindow.Title);
		}

		protected virtual void	OnEnable()
		{
			Utility.RestoreIcon(this, NGComponentReplacerWindow.TitleColor);

			Metrics.UseTool(16); // NGComponentReplacer
			...
			this.converters = Utility.CreateNGTInstancesOf<TypeConverter>();
		...
		{
			EditorGUILayout.HelpBox("Save & backup your project before using " + NGComponentReplacerWindow.Title + ", this is highly unstable.", MessageType.Warning);
			...
			Rect	r = new Rect(0F, 105F, this.position.width, 1F);
		...
		private void	OnGUIModel()
		{
			...
			GUI.BeginGroup(r);
			{
				...
				r.x = 15F;
				r.y = -this.targetScrollbar.Offset;
				r.width = this.targetMaxWidth;
				...
				r.y = -this.replaceScrollbar.Offset;
				r.width = this.replaceMaxWidth;
				...
				for (int i = 0; i < this.links.Count; i++)
				{
					Vector2	a = new Vector2(this.targetMaxWidth + 15F + NGComponentReplacerWindow.DeleteAreaWidth,
											NGComponentReplacerWindow.FieldHeight * .5F - this.targetScrollbar.Offset + this.links[i].target * NGComponentReplacerWindow.FieldHeight);
					Vector2	b = new Vector2(this.position.width - this.replaceMaxWidth - 15F - NGComponentReplacerWindow.DeleteAreaWidth,
											NGComponentReplacerWindow.FieldHeight * .5F - this.replaceScrollbar.Offset + this.links[i].replace * NGComponentReplacerWindow.FieldHeight);
		...
		private void	SetTarget(Type type)
		{
			...
			{
				EditorUtility.DisplayDialog(NGComponentReplacerWindow.Title, "Transform and RectTransform can not be choosen.", "OK");
				return;
			}
		...
		{
			Utility.AddNGMenuItems(menu, this, NGComponentReplacerWindow.Title, NGAssemblyInfo.WikiURL);
		}
	}
}